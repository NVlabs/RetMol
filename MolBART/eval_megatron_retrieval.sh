...
megatron_options=" \
        ...
        --load /mol-gen/drug/models/megamolbart/checkpoints
"
...
custom_train_options=" \
                --stage 1 \
                --train_from pretrain \
                --model_ckpt_itr 134000 \
                --attr logp-sa \
                --attr_offset 0 \
                --data_source jtnn \
                --enumeration_input false \
                --retriever_rule random \
                --pred_target nearestn \
                --n_retrievals 10 \
                --n_neighbors 100
                "


run_cmd="deepspeed --include localhost:0 --master_port=${MASTER_PORT} megatron_molbart/eval_retrieval.py $@ ${full_options} ${custom_train_options}"
echo ${run_cmd}
eval ${run_cmd}
