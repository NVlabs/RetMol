...
def parse_args(extra_args_provider=None, defaults={},
    ...
    # JACK: custom function added by me
    parser = _add_mol_gen_exp_args(parser)
    # Custom arguments.
...
# JACK: this is a custom function that I have added
def _add_mol_gen_exp_args(parser):
    ...
    group = parser.add_argument_group('molecule generation experiment specific arguments')
    group.add_argument("--stage", default=1, type=int)
    group.add_argument("--train_from", default='pretrain', 
                        help='load a pretrained megamolbart (pretrain) or \
                                stage 1 trained megamolbart (stage1) for stage 1 or 2 training')
    # parser.add_argument("--pretrain_model_path", 
    #                     default='MolBART/megatron_molbart_v3_attr-logp-sa_checkpoint')
    group.add_argument("--model_ckpt_itr", default=50000, type=int, 
                        help='only used when in stage 2 training')
    group.add_argument("--attr", default='logp-sa', help='only need in stage 2')
    group.add_argument("--attr_offset", default=0, type=float, help='only need in stage 2')
    group.add_argument("--data_source", default='tdc', help='tdc or jtnn')
    group.add_argument("--enumeration_input", default='true', type=str2bool)
    group.add_argument("--retriever_rule", default='topk', help='topk or random')
    group.add_argument('--pred_target', default='reconstruction', 
                        help='reconstruction, canonical, or nearestn (nearest neighbor)')
    group.add_argument("--n_retrievals", default=10, type=int)
    group.add_argument('--n_neighbors', default=100, type=int, 
                        help='this is used with the random retriever rule only, \
                                for selecting an initial nearest neighbor set for random sampling later')
    return parser